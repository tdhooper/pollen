...
function icosphere(subdivisions) {
  ...
  function getMidpoint(a, b, c) {
    var indicies = [a, b];
    if (c !== undefined) {
      indicies.push(c);
    }
    var id = indicies.sort().join('-');
    if (positionMap.hasOwnProperty(id)) {
  ...
  faces.forEach(function(face) {
    ...
    var b = face[1];
    var c = face[2];
    var center = getMidpoint(a, b, c);
    var ab = getMidpoint(a, b);
    var bc = getMidpoint(b, c);
    var ca = getMidpoint(c, a);
    schwarzFaces.push([a, ab, center]);
    schwarzFaces.push([ab, b, center]);
    schwarzFaces.push([b, bc, center]);
    schwarzFaces.push([bc, c, center]);
    schwarzFaces.push([c, ca, center]);
    schwarzFaces.push([ca, a, center]);
  });
